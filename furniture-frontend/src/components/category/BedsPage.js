...
import './Lighting.css'; // Reuse the same CSS or create a new one if needed
...
const BedsPage = () => {
  ...
  const [sortOrder, setSortOrder] = useState('default'); // For sorting products
  const [filter, setFilter] = useState('Bed'); // For filtering by Bedding category
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/products/view?category=Bed`, {
        method: 'GET',
        headers: {
      ...
      console.log('Fetched decoration products:', data); // Debug: Check fetched data
      setProducts(data);
    } catch (error) {
  ...
  const addToWishlist = async (productId) => {
    ...
    try {
      ...
      const addResponse = await axios.post(`${BASE_URL}/api/wishlist`, { productId }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="product-page">
      ...
      <h1 className="page-title">Bedding Collection</h1>
