...
const FurniturePage = () => {
  ...
  const [filter, setFilter] = useState('Furniture'); // State for filtering to decoration category
  ...
  const fetchProducts = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/products/view?category=Furniture`, {
        method: 'GET',
        headers: {
  ...
  return (
    <div className="product-page">
      ...
      <h1 className="page-title">Furniture Collection</h1>
      ...
      <div className="sort-filter-bar">
        ...
        <select onChange={(e) => setFilter(e.target.value)} value={filter}>
          <option value="Decoration">Furniture</option>
        </select>
      </div>
